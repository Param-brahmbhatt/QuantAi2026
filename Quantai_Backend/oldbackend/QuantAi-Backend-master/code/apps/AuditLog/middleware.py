...
from apps.AuditLog.context import set_actor
...
class AuditlogMiddleware:
    ...
    def __call__(self, request):
        ...
        if hasattr(request, "user") and request.user.is_authenticated:
            context = set_actor(actor=request.user, remote_addr=remote_addr, geo_data = request.geo_data, user_agent=request.user_agent)
        else:
